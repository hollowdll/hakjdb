...
func showServerInfo() {
	...
	info.WriteString(fmt.Sprintf("server_version: %s\n", res.GeneralInfo.ServerVersion))
	...
	if res.GeneralInfo.TlsClientCertAuthEnabled {
		info.WriteString("tls_client_cert_auth_enabled: yes\n")
	} else {
		info.WriteString("tls_client_cert_auth_enabled: no\n")
	}
